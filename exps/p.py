...
def run_exp(R,N,Corners,Reps):
    for i in range(len(R)):
        ...
        return [np.mean(nonten_results[:,0]),np.std(nonten_results[:,0])/np.sqrt(reps),np.mean(nonten_results[:,1]),np.std(nonten_results[:,1])/np.sqrt(reps)
                ,np.mean(amcomp_results,0)
                ,[0,0],[0,0]]
                # ,np.mean(silrtc_results,0),np.mean(tncomp_results,0)]
...
for p in reversed(range(8,9)): #(4,9)
    ...
    results[p-4,3] = res[2]
    results[p-4,4] = res[3]
    results[p-4,5] = res[4][0]
    results[p-4,6] = res[4][1]
    results[p-4,7] = res[5][0]
    results[p-4,8] = res[5][1]
    results[p-4,9] = res[6][0]
    results[p-4,10] = res[6][1]
    np.save('results/p.npy', results)
